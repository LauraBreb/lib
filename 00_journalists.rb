def journalists_number (journalists)
    ...
    puts "There are #{number} journalists listed"
    return number
end

def shorter_handle (journalists)
    handle_length = journalists.map {|handle| handle.size.to_i}
    handle_length.sort
    shorter_length = handle_length[0]
    puts journalists.find {|handle| handle.size == shorter_length}
end

def lenght_select_handle (journalists)
    puts journalists.count {|handle| handle.size == 6}
end

def maj_handle (journalists)
    puts journalists.count{|handle| /[[:upper:]]/.match?(handle[1])}
end
...
def sort_handle_by_size (journalists)
